...
if ($WindowsPreview) {
  if ($VersionTag -notmatch '-rc\.\d+$') {
    throw 'Windows-only publication requires an rc tag'
  }
  $platforms = @($platforms | Where-Object platformId -eq 'win-x64')
}

if (Test-Path -LiteralPath $outputPath) {
  throw 'OutputDir already exists; immutable release output is not overwritten'
}
...
foreach ($platform in $platforms) {
  ...
  if ($WindowsPreview) {
    ...
    if (
      ...
      $verification.archiveSha256 -ne $actualHash -or
      $verification.stagingSmoke -ne 'passed' -or
      $verification.freshExtractionSmoke -ne 'passed' -or
      $verification.offlineNetwork -ne 'non-loopback TCP and DNS blocked in Node processes'
    ) {
      throw "Verification evidence is inconsistent for $($platform.platformId)"
    }
  }
...
$manifest = [ordered]@{
  ...
  releaseScope = if ($WindowsPreview) { 'windows-only-preview' } else { 'three-platform' }
  artifactDistribution = if ($WindowsPreview) { 'local-only' } else { 'remote-release' }
...
if ($WindowsPreview) {
  Write-Host "Verified release set created: $outputPath (WindowsPreview=$WindowsPreview)"
}
else {
