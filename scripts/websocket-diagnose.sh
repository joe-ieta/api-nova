...
if curl -s -I http://localhost:9001/socket.io/ | grep -q "400 Bad Request"; then
    echo "✅ Socket.IO端点可访问"
else
...
# 3. 检查静态文件
echo "3. 检查测试页面..."
if curl -s http://localhost:9001/websocket-test.html | grep -q "WebSocket"; then
    echo "✅ 测试页面可访问"
    echo "   📱 请访问: http://localhost:9001/websocket-test.html"
else
    echo "❌ 测试页面不可访问"
fi
...
echo ""
echo "🚀 下一步操作建议："
echo "1. 访问测试页面: http://localhost:9001/websocket-test.html"
