...
## 13. v1.7.5-rc.2 单次发布例外（2026-09-08）

用户已明确批准本次改为 Windows x64 单平台预发布，并要求实际发布物不上传远端。仅对 `v1.7.5-rc.2` 生效：

- 本版本范围为 `windows-only-preview`，不代表三平台完整正式发布；Linux x64/ARM64 不构建、不伪造验证结果。
- 保留仓库全部门禁、Windows 原生离线包、暂存与最终 ZIP 新解压复验、校验清单、版本文档和精确 Git SHA 要求。
- ZIP、Manifest、SHA256 与运行镜像仅保留本地；不创建包含附件的远端 Release，不运行上传产物的 CI 工作流。
- 允许在 Windows 门禁全部通过后创建并推送注释预发布标签；远端只推送代码、文档和该标签。
- 本地归档位于 `api-nova-release-archive/v1.7.5-rc.2/`；运行副本位于 `api-nova-release-preview/v1.7.5-rc.2/win-x64/`。
- 不修改已有 `api-nova-release`，不更新三平台 `CURRENT_RELEASE.json`；现有运行数据保持不动。
- Manifest 明确标记单平台、本地分发，不能被三平台晋升脚本接受。
- 后续版本仍执行本文件的三平台完整发布要求，除非再次获得明确的范围变更授权。
