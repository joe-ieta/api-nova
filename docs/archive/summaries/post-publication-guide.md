# 🎉 ApiNova Server - 发布完成使用指南

恭喜！您的 `api-nova-server` 已经成功发布到 NPM！以下是完整的使用指南。
...
# 从本地文件启动并监控变化
api-nova-server --transport http --openapi ./my-api.yaml --watch
```
...
# 使用 PM2 管理
pm2 start "api-nova-server --transport http --openapi https://api.example.com/openapi.json" --name "my-mcp-server"
```
...
```bash
# 开发环境自动同步
api-nova-server --transport http --port 9022 --openapi ./dev-api.yaml --watch
```

访问 `http://localhost:9022` 进行交互式 API 测试。

### 场景 3: 微服务集成

```bash
# 为每个微服务启动独立的 MCP 服务器
api-nova-server --transport streamable --port 9001 --openapi https://user-service.com/openapi.json
api-nova-server --transport streamable --port 9002 --openapi https://order-service.com/openapi.json
api-nova-server --transport streamable --port 9003 --openapi https://payment-service.com/openapi.json
```
...
```bash
# 检查端口使用
netstat -an | findstr :9022  # Windows
lsof -i :9022                # macOS/Linux
...
```bash
# 启用详细日志
set DEBUG=api-nova:*        # Windows
export DEBUG=api-nova:*     # macOS/Linux
...
- **Issues**: https://github.com/yourusername/api-nova-server/issues
- **Discussions**: https://github.com/yourusername/api-nova-server/discussions
...
# 使用 PM2 进程管理
npm install -g pm2
pm2 start "api-nova-server --transport http --openapi https://api.prod.com/openapi.json" --name "mcp-api"
pm2 save
pm2 startup
```
...
```bash
# 限制访问地址
api-nova-server --transport http --host 127.0.0.1 --openapi ./internal-api.yaml

# 使用环境变量存储敏感 URL
export API_URL="https://api.company.com/openapi.json?token=SECRET"
api-nova-server --openapi $API_URL
```
...
api-nova-server --openapi ./cached-api.json
```
...
**🎉 恭喜！您现在已经掌握了 ApiNova Server 的完整使用方法。开始让您的 API 与 AI 无缝集成吧！**
