...
全局编码门禁为 OBS-GATE-01，已通过。OBS-TP-01/02/03/04/05/08/09 已满足各自退出条件；OBS-TP-06/10/11 进行中，OBS-TP-07 已就绪。2026-09-13 事件历史 OBS-API-16 已 VERIFIED，Outbox 仍未完成。后续按依赖推进，不重复请求计划确认。
...
当前 DONE：OBS-API-17~25 与 OBS-PUSH-02 已完成。订阅、测试事件、投递查询、人工重投及 Webhook Worker 专项覆盖授权范围、签名游标、尝试分页、脱敏、强制幂等、终态约束、密钥引用、DNS 结果复核、固定地址连接、HMAC 验签、Retry-After、六次有限重试和租约恢复；事件、投递、审计与幂等收据保持同事务，网络发送在事务外执行。Worker 默认关闭且未部署。
...
3. TP10-B03：依赖 B02，补齐桶级重算、覆盖/保留期、恢复与回填，提供已完成快照的读取，替换按需模式前单独验收（持久读路径兼容回退已接入）。
4. TP10-B04：依赖 B02/B03 及持久事件，提供桶修订事件与状态报送；服务器存活与覆盖仍需独立证据。

B01/B02 已完成并通过专项及联合回归；TP10-B03 已完成验收，B04 仍待依赖完成。TP-06/10 仍进行中，任务包计数保持 DONE=7、IN_PROGRESS=2、READY=2、BACKLOG=5；不提前完成 TP-10 或推送任务包。
...
| TP10-B02 | B01 | DONE | 已接入持久事务版本条件、贡献引用、待重算桶标记；并发、重启恢复边界和失败回滚验收已闭环（`test-call-observability-bucket-projection-recovery.cjs`：4/4） |
| TP10-B03 | B02 | DONE | 有效贡献重算、覆盖/保留期/回填、持久快照读取与边界验收（兼容持久桶回退路径已接入）；新增场景 2 例并补充 malformed marker 与多桶回算断言，6/6 执行通过 |
| TP10-B04 | B02/B03、持久事件 | IN_PROGRESS | 桶/采集状态事件和授权事件历史已接入，Windows 文件身份回归已修复；可恢复分发及实际推送待完成，详见执行状态第 43 节 |

B02 实施时需在存储事务内重新读取已投影数据库版本，原子更新贡献引用和待重算标记；重复/过期输入不产生额外工作。失败必须全部回滚，多个进程不能依据同一旧版本重复提交。覆盖回填与明细过期不得靠 B01 的无状态规划自动推断。
...
## 2026-09-14：已确认远端主链路的整合口径

用户确认保留 origin/main 7a7fc44 已闭环工作。本地重复聚合/订阅/投递内核由远端替代，保留本地独有查询、缓存、origin 和 MCP 修复。TP11/TP12 沿用远端 DONE，TP07 沿用本地 DONE，当前包口径 DONE=10、IN_PROGRESS=2、BACKLOG=4。公开编号并集26/28不等于合并后验收，结果以本轮实际回归为准；AVAILABLE=0。

接续重点为整合回归、TP13 Socket.IO、TP10/14 真实状态与治理、TP06/16 平台矩阵、TP15 身份审计/旧消费者收敛。不要重新开发远端已经提供的订阅管理、投递查询和人工重投。最新状态以 runtime-observability-completion-review.md 2.0 为准，历史章节保留但不覆盖本节。