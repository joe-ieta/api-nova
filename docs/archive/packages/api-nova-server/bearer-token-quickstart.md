...
> Reason: Historical authentication quickstart no longer defining the product path.

## 概述

ApiNova 支持 Bearer Token 认证，可以保护你的 API 访问。支持三种配置方式：
...
```bash
# 直接指定 Bearer Token
api-nova-server --auth-type bearer --bearer-token "your-token-here" -o ./api.json

# 使用 SSE 传输 + Bearer Token
api-nova-server -t sse -p 3323 --auth-type bearer --bearer-token "your-token-here" -o ./api.json
```

#### 环境变量 Token
```bash
# 使用默认环境变量 API_TOKEN
api-nova-server --auth-type bearer --bearer-env API_TOKEN -o ./api.json

# 使用自定义环境变量
api-nova-server --auth-type bearer --bearer-env CUSTOM_TOKEN -o ./api.json
```
...
```bash
# 使用配置文件
api-nova-server -c config.json

# 或者使用完整路径
api-nova-server --config ./config/production.json
```
...
```bash
# 使用配置文件
api-nova-server --config production.json
```
