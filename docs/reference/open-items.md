...
- 外部环境验收：`EXT-01` 到 `EXT-10`、`ENV-01`、`SEC-OPS-01`、`SEC-DEP-01`。
...
- 以下项建议先挂起并作为回归后置条件：`EXT-01`~`EXT-10`、`ENV-01`、`SEC-OPS-01`、`SEC-DEP-01`、`SEC-AUTH-01`、`SEC-POLICY-01`、`DEV-01`、`AUDIT-01` 的检索/展示阶段、以及 `DEV-01` 依赖的 UI 交互联调。

当关闭某个验收项时，请记录责任人、目标环境、对应提交、执行命令、结果及脱敏证据位置。凭据必须保存在经批准的本地配置/密钥管理中，不要写入该文档或 Git。联调夹具证据不得替代未执行的外部用例。

2026-09-08 数据库验证更新：本地真实隔离 PostgreSQL 与 SQLite 均通过 43 张业务表初始化、零数据/零结构差异、事务持久化及完整 API 启动；九阶段回归门禁通过。原 `EXT-11` 本地数据库阻塞项已关闭，证据见 [清理审查记录](../audits/2026-09-08-persistence-cleanup.md)。生产部署、备份恢复与跨平台验收仍未关闭。
...
恢复外部联调前需提供：受信任 issuer/JWKS URL、Gateway/MCP 公网 URL 与反向代理配置、测试凭据本地路径。完成真实登录/同意流程、客户端刷新与重连后，再次执行 `npm run verify:runtime-closure`。本地 43 表 PG/SQLite 门禁已通过；安全夹具仍直接种子化路由快照，不覆盖从注册、治理、发布到托管部署的端到端流程。
...
- 发布前确认应用与迁移 CLI 共用的数据库环境、目标空库及备份/恢复流程，再初始化 43 表结构。不支持历史结构原地升级；当前本地验证未修改现有业务数据库。
...
已完成行为与测试计数记录于上述设计/验收文档，不应在此文件作为未完成项处理。历史 7/38/40 表结果不代表当前模型；本次 43 表 PG/SQLite 本地验证与受控安全联调也不关闭外部身份提供商、托管发布、UI/平台及生产运维验收。
