...
| 安全 SEC-C3/C4/E1 | 稳定文件源及 Gateway 配置激活已验证 | 管理Reload/意图结果审计已验证；已新增MCP可信映射与显式单跳共享Resolver；已补单SELECT归属读取、映射生成器与进程内装配接线；发布/profile/upstream非整体一致事务；Watch、多进程、一致仓储读取及托管启动链接入、运行中撤销与自动逐跳/DNS网络边界仍待完成 |
