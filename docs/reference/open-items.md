...
| 安全 SEC-C3/C4/E1 | 稳定文件源及 Gateway 配置激活已验证 | 管理Reload/意图结果审计已验证；已新增MCP可信映射与显式单跳共享Resolver；已补归属/发布/最新profile单SELECT读取与装配；已补上游返回与捕获链的跨源核验；已补激活时版本/失效guard；同源版本与后续验证/激活仍非完整CAS或整体一致事务（完整/跨进程CAS经[PROD-06核定](../audits/2026-09-26-prod-06-cas-scope-determination.md)属并发强化候选、非原WP70出口，未批准不排期）；Watch、多进程、一致仓储读取及托管启动链接入、运行中撤销与自动逐跳/DNS网络边界仍待完成 |
