...
Changesets 是一个用于管理 monorepo 版本控制和发布的工具，特别适合我们当前的 ApiNova 项目。它可以帮助我们：
...
│   └── README.md           # 使用说明
├── packages/
│   ├── api-nova-server/  # 主服务包
│   ├── api-nova-parser/  # 解析器包
│   └── api-nova-ui/      # UI包（可选）
└── package.json            # 根包配置
```
...
- **api-nova-server**: 主包，独立版本管理
- **api-nova-parser**: 解析器包，独立版本管理
- **api-nova-api**: 私有包，不发布（ignore）
...
### 2.1 当前状态分析
- 当前版本：api-nova-server@1.0.9, api-nova-parser@1.0.5
