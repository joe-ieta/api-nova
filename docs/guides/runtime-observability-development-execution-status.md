...
SQL.js CAS/坏行/重开专项4/4、真实Windows child start→taskkill stop与exit 7、注入error→lost事件hook 3/3、状态投影1/1、ProcessManager相邻认证/进程回归5 suites/32 tests、API type-check与build均通过。没有新增schema/migration，复用`RuntimePipelineStateEntity`；没有生产部署或外部环境验收。OBS-10父包已按限定范围DONE；后续OBS-10-02A读模型证据见第23节。OBS-13-01后续已拆为A/B/C；A已限定完成，B现READY，详见第23节。
