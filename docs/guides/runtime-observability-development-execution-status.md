...
| 实时流 | 有界授权页流与慢客户端/断线恢复（OBS-13-02限定）已有专项；长时soak、完整状态快照、旧 UI 迁移与跨平台矩阵未闭合 | TP13 |
| 状态/治理 | 真实存活、长期覆盖、配额、完整策略与恢复；新事件/正文策略、默认关闭正文GC、授权覆盖、管理审计30天与投递/receipt墓碑有界保留（OBS-14-03D限定，物理删除默认关闭）已验证 | TP10/14 |
...
SQL.js CAS/坏行/重开专项4/4、真实Windows child start→taskkill stop与exit 7、注入error→lost事件hook 3/3、状态投影1/1、ProcessManager相邻认证/进程回归5 suites/32 tests、API type-check与build均通过。没有新增schema/migration，复用`RuntimePipelineStateEntity`；没有生产部署或外部环境验收。OBS-10父包已按限定范围DONE；OBS-13-01的A、B1、B2与C现均限定完成，聚合OBS-13-01可限定DONE；OBS-13-02解锁READY，TP13/父OBS-13因长期客户端与跨环境出口仍保持IN_PROGRESS，详见第23节。
