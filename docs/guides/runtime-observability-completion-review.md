...
| TP13 | IN_PROGRESS | 授权持久事件分页流、调用事实快照接续、每页权限复验、ACK 背压与隔离模式已实现；调用事实UI已接入；全局状态快照、其余UI与长期/跨平台传输矩阵仍待验收；已有真实Socket.IO回环专项不替代完整矩阵 |
...
| TP10 状态与覆盖 | 管理心跳、本实例路由和诊断UI已接入；仍需业务真实存活、实时在途及完整历史维度/状态闭环 | 明确心跳、生命周期与采集新鲜度的区别；不把保留调用快照解释为全量历史 |
