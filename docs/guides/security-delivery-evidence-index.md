...
| SEC-F3-02C1d2b3a | [网络边界合同§4.3–4.5](./security-header-network-boundary-contract.md) | **IN_PROGRESS/未验收**：独立Gateway host-only generation/issuer capability端口正在实现；缺失默认off，不从request/config/env/file自动导入，不改RuntimeModule/Registry且不启用生产路径。 |
| SEC-F3-02C1d2b3b | [网络边界合同§4.3–4.5](./security-header-network-boundary-contract.md) | **WAIT_DEP**：依赖D2b3a/D2b2/D3；Registry/evidence生产组合与同代Snapshot/proof/epoch/readSignal验收待实现。 |
