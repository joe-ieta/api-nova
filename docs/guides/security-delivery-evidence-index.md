...
| SEC-F3-02B2 | [网络边界合同§4.3](./security-header-network-boundary-contract.md) | **IN_PROGRESS**：固定获批IP单跳直连、原Host/SNI/证书保留、代理/外部Agent拒绝及写出前peer复核正在实施；本叶只限单跳transport，不实现redirect、逐跳凭据或撤销状态机，不能解锁F3C。 |
