...
| SEC-F3-02C2b2b2 | [网络边界合同§4.3–4.5](./security-header-network-boundary-contract.md) | **READY**：b2b1/C2a/C2b1/C2b2a/D2b2依赖已闭合；同一authority/deadline/Signal内的逐跳目标选择、DNS/peer/TLS、目标凭据及真实网络验收待实现，默认仍为single-hop。 |
| SEC-F3-02C2b3 | [网络边界合同§4.3–4.5](./security-header-network-boundary-contract.md) | **WAIT_DEP**：依赖C2b2b2/D3；Transformer显式可信host配置与生产入口矩阵待实现，默认仍为single-hop。 |
...
| SEC-F4-01 | 本索引及SEC当前叶子出口逐项映射 | **限定执行已完成/持续维护**：索引按当前109个SEC叶子或明确标注的聚合旧ID维护；ID/链接校验不改变技术父包状态，也不自动使 F4-02 READY。 |
