...
2026-09-14 对齐口径：OBS-TP-01~05、07~09、11、12 为 DONE，共 10 包；06、10 为 IN_PROGRESS；13~16 为 BACKLOG。OBS-API-01~26 在限定契约内 VERIFIED，27/28 policies 未实现，AVAILABLE=0。Webhook 包已闭环但 Worker 默认关闭、未部署；新 Socket.IO 协议未实现。任务包完成不等于跨包 AC 或整体平台、性能、部署验收完成。
...
退出：有正常/拒绝/缓存/重试/Tool 错误的契约夹具及身份/计数断言，当前格式缺失字段有明确 missingFields/unknown 规则，不导入旧格式。不能用时间接近猜测调用同一性。
...
交付 OBS-API-03~10。提供元数据、分侧正文、trace、调用者/来源和标签修改，执行统一授权及敏感读取审计，返回明确正文状态和当前数据集覆盖范围。
...
已实现边界：OBS-API-01/02/11~15 的限定查询契约已验证；Store 持久聚合重算、缓存三态统计、overview、依赖归因、授权服务器状态及安全事件快照桥接已合并。不是待建 B02 持久内核，也不另建本地聚合器。TP-10 仍 IN_PROGRESS：真实心跳/实时在途、完整历史维度与整包 AC-11/19 不能由有界查询代替。
...
已完成包内边界：授权事件历史、固定高水位、签名游标、保留期错误及白名单响应；Outbox 按提交 sequence 的半开修订选路，具备持久 delivery 去重、租约恢复及无缺口分发水位。网络发送、重试与死信归 TP-12。origin 与 overview 已签发调用事实快照桥接不改变原有事件类型、1000 条扫描上限、complete 或 410 约束。
...
已完成包内边界：OBS-API-17~25 与 OBS-PUSH-02 已完成。订阅、测试事件、投递查询、人工重投及 Webhook Worker 专项覆盖授权范围、签名游标、尝试分页、脱敏、强制幂等、终态约束、密钥引用、DNS 结果复核、固定地址连接、HMAC 验签、Retry-After、六次有限重试和租约恢复；事件、投递、审计与幂等收据保持同事务，网络发送在事务外执行。Worker 默认关闭且未部署。
...
本包仍为设计目标。现有 overview→events 仅桥接调用事实，不能据此认定实时订阅已完成。
...
本包仍为 BACKLOG；OBS-API-26 已完成真实持久状态的限定读取，不代表统一治理已完成。OBS-API-27/28 未实现。
...
completion-review 是当前状态唯一汇总，execution-status 是证据索引与真实剩余清单。任务状态使用 BACKLOG、READY、IN_PROGRESS、BLOCKED、REVIEW、DONE、DEFERRED，与现有项目习惯一致。全局审批门禁单独记录，不把“等待计划确认”误报为实现技术故障。

任务 DONE 必须同时具备：已交付的代码/配置/初始化基线或该包明确的文档产物；对应验收结果；适用测试记录；更新的 API/设计与破坏性调整说明；没有会破坏依赖方契约的未决问题。仅计划、写入代码、运行过但失败的测试不能满足 DONE。

执行证据记录在 execution-status，不在本文追加历史流水。每次更新填写实际受影响文件、完成项、测试命令/结果、证据路径、数据库影响、待办与下一步。未执行测试写“未执行”，日期不代表测试通过。回归导致契约失效时任务重开，不保留虚假的 DONE。

TP-11/12 的 DONE 按各自已验证交付边界保留；全平台、完整故障矩阵及跨包验收归 TP-15/16，不反向混淆包内完成状态。批准的退出条件仍保留，未覆盖的整体场景应在证据索引中注明。
...
批准保留目标不随实现缩减：投递记录目标 30 天，而当前 Outbox 生成 delivery 的到期时间受事件到期及 14 天上限约束；治理包需明确记录并处理该差异，不能修改需求来消除缺口。
