...
当前 IN_PROGRESS：OBS-API-17~21 订阅管理以及 OBS-API-22~25 测试事件、投递查询和人工重投已完成。专项验收覆盖授权范围、签名游标、尝试分页、脱敏、强制幂等和终态约束；事件、投递、审计与幂等收据保持同事务。实际密钥解析、DNS 后复核、签名发送和有限重试 Worker 仍未实现。
