...
全局编码门禁为 OBS-GATE-01，已通过。OBS-TP-01/02/03/04/05/07/08/09 已满足各自退出条件；OBS-TP-06/10/11 进行中。后续按依赖推进，不重复请求计划确认。
...
当前节点：OBS-API-01/11/12/13 已 VERIFIED；TP10-B01 桶键及修订规划内核已完成，专项 50/50、联合 430/430 PASS。TP-10 仍 IN_PROGRESS，下一节点 B02 为持久表及事务投影，持久重算、状态/总览与推送仍待实施。
...
## TP-09 标签节点与新增条件请求边界（2026-09-09）

OBS-API-09 已编码，22 项专项及 315 项联合、API 构建通过；实现完整登记资产权限、事务内 If-Match、编辑版本、字段限制与管理审计。但额外框架级检查发现：详情的档案专用 ETag 可能错误触发变化中统计响应的 304。已请求仅分离档案令牌与完整响应 ETag 并补回归，尚未修正；08/09 暂 IMPLEMENTED，整包继续 IN_PROGRESS。

本节点不因现有测试全绿而提前收口。下一步先完成该边界修正、真实路由条件请求回归与节点提交，再验收 TP-09 八接口的包内退出标准。TP-10/11 已 READY，随后依序启动聚合/状态和事件历史；全局入口审计、应用启用、治理和全平台矩阵仍按原依赖负责。

## TP-09 收口与 TP-10 接续（2026-09-09）
...
## TP-10 统计纯计算节点（2026-09-09）

已交付有界指标计算内核，复用既有 scope 和严格时间窗解析，补齐最新数据库修订去重、来源/调用者集合、下界字节、状态分母和固定延迟直方图。24 项专项与 358 项联合通过，API 构建通过；三项初次夹具错误及获批修正见执行台账第 35 节。

本节点不交付新 Endpoint、数据库聚合 worker 或桶事件；01、03~10 九接口仍 VERIFIED。TP-10 保持 IN_PROGRESS。接续顺序为 OBS-API-11 授权汇总查询及契约验证，再接时间序列/分组与持久桶/迟到更正，随后依赖、状态与总览。查询必须在计算前落实授权、TTL 和当前快照，不以组件纯计算测试替代真实数据库/HTTP 验收。

## TP-10 授权统计汇总节点（2026-09-09）

OBS-API-11 接入只读修订快照与有界计算，覆盖权限/TTL/时间/scope 前置筛选、参数绑定、来源同资产关联及 5000/5001 边界。20 项实际 HTTP/SQL.js/Swagger、能力 14 项与联合 378 项通过，API 构建通过。跨服务器 sourceId 测试预期获批修正，既有身份规则保持不变。

01、03~11 共十条 VERIFIED，十八条 PLANNED；根应用未启用。接续 OBS-API-12/13 时间序列/分组与相应能力边界，持久桶、迟到贡献撤回及更新事件仍按本包退出条件完成。queryMode 只证明保留明细快照，不证明长期历史、heartbeat 或聚合 worker 已运行。TP-10 保持 IN_PROGRESS，包级完成数不变。

## 时间序列/分组节点与后续依赖（2026-09-09）

1. 当前节点：OBS-API-12/13 生产代码、DTO、Swagger、能力声明及 26 项专项已编写，API build、26 项专项、20 项汇总通过；能力回归 10/14，通过前不提升 Endpoint 状态。
2. 紧接任务：仅修正能力测试批量替换造成的 4 处断言及过时标题，再执行 3 脚本专项和加入新脚本的 19 脚本联合回归。完成后更新验收证据，按需提交及普通推送。
3. TP-10 后续：明确持久聚合的桶键、幂等修订、迟到完成、保留期和重算策略；在真实覆盖及心跳依据上实现总览、依赖、服务器状态。按需统计不能充当长期桶或健康证据。
4. TP-07/11 后续：接入持久事件历史、主动报送与消息记录，复用修订/去重语义和授权边界；不得将当前查询能力当作推送已经实现。
5. TP-12~16：继续按原任务包依赖推进，监控大屏可后续实现；优先使用已明确的窗口、覆盖、截断和未知状态字段，避免先做展示再倒推错误统计语义。

任务包计数保持 DONE=7、IN_PROGRESS=2、READY=2、BACKLOG=5。当前节点未提交，不调整原有完成任务包或历史验收记录。

## 时间序列/分组验收及后续节点（2026-09-11）

2026-09-09 初轮断言错误经授权仅修正测试后解决。专项 60/60、19 脚本联合 404/404 PASS，0 fail/cancelled/skipped；OBS-API-12/13 已 VERIFIED。此前初轮待修正清单保留为历史记录，不再表示当前阻塞。

接续 TP-10 持久聚合工作分为四个节点：

1. TP10-B01：桶键与修订规划纯内核。固定 UTC 桶及资产/origin/scope/timeBasis 隔离，按数据库修订识别重复/过期输入，计算新增、移出及需重算桶集合；专项验证后提交。
2. TP10-B02：依赖 B01，接入持久表、贡献引用、事务版本条件与可恢复重算队列；不以规划结果本身作为已提交状态。
3. TP10-B03：依赖 B02，补齐桶级重算、覆盖/保留期、恢复与回填，提供已完成快照的读取，替换按需模式前单独验收。
4. TP10-B04：依赖 B02/B03 及持久事件，提供桶修订事件与状态报送；服务器存活与覆盖仍需独立证据。

B01 已完成并通过专项及联合回归，B02 READY，B03/B04 待依赖完成。TP-06/10 仍进行中，任务包计数保持 DONE=7、IN_PROGRESS=2、READY=2、BACKLOG=5；不提前完成 TP-10 或推送任务包。

## TP10-B01 完成与 B02 接续清单（2026-09-11）

时间序列/分组节点 ef66443 已提交推送。本轮继续完成 B01，新增 26 项测试，API build、50 项专项和 430 项联合回归全部通过。

| 子节点 | 依赖 | 状态 | 当前交付或验收目标 |
| --- | --- | --- | --- |
| TP10-B01 | 已完成指标内核与统计查询 | DONE | 稳定桶键、数据库版本精确比较、重复/过期判定、最多 32 桶的重算规划 |
| TP10-B02 | B01 | DONE | 持久贡献引用、桶状态、事务版本条件、失败回滚、并发与重启恢复 |
| TP10-B03 | B02 | IN_PROGRESS | 有效贡献重算、覆盖/保留期/回填、持久快照读取与边界验收 |
| TP10-B04 | B02/B03、持久事件 | PLANNED | 桶修订事件、事件历史和主动报送，不伪造运行健康 |

B02 实施时需要在存储事务内重新读取已投影数据库版本，原子更新贡献引用和待重算标记；重复/过期输入不产生额外工作。失败必须全部回滚，多个进程不能依据同一旧版本重复提交。覆盖回填与明细过期不得靠 B01 的无状态规划自动推断。

父任务包状态和数量不变。B01 完成仅指纯规划内核，不表示 TP-10、持久聚合或任何推送已完成。

## 本轮完成情况复核与未完成标注（2026-09-11）
...
OBS-TP-01/02/03/04/05/08/09 为 DONE；OBS-TP-06/10 为 IN_PROGRESS；OBS-TP-07/11 为 READY；OBS-TP-12~16 为 BACKLOG。共 9 个任务包未完成。READY 只表示可开始，不能计入完成。

TP10-B01 已完成纯规划，不代表持久聚合完成。B02 应先核对已有桶/贡献实体后接入事务投影，不从零再创建重复实体；B03/B04、统一状态/剩余指标、事件历史、Webhook、WebSocket、治理与完整验收仍待推进。现有调用事件入库和旧监控模块是基础，不等于新报送闭环。

大屏 UI 延后不影响后端状态/事件契约仍为必做事项。平台、性能、完整启动与部署验收没有通过前，不将独立模块测试扩大为 AVAILABLE。

## 并发开发后的当前执行入口（2026-09-11）

本节及最新执行台账覆盖前述历史节点状态：DONE=8、IN_PROGRESS=3、READY=0、BACKLOG=5。TP-07 五类入口和自身健康 telemetry 排除已验收；TP-11 从 READY 进入 IN_PROGRESS。B02 的持久事务投影已完成，B03 内部重算、快照和可选后台已实现，但长期治理/覆盖/HTTP 持久读取仍待完成。

OBS-API-02/14/15/16/26 本轮 VERIFIED，新 HTTP 总数为 17 VERIFIED、11 PLANNED。根模块和默认关闭的采集/聚合/Outbox 开关为 TP-15 接入准备，pipeline/status 为 TP-14 准备，均不据此完成下游硬依赖或宣称部署 AVAILABLE。主任务责任保持：TP-07 内部入口、TP-11 事件/Outbox、TP-12 Webhook。

实际验证和剩余 REM 清单见 runtime-observability-completion-review.md；API 29 脚本 507/507、Parser 审计 103/103 PASS。接续重点为 B03 长期治理和 TP-11 统一事件收口、TP-12 网络投递、TP-13 实时通道，并行保持 TP-06 完整矩阵及平台性能验收。
