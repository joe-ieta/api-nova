...
实现快照（2026-09-24）：F3-02A、B1、B2及B3a/b/c均按限定出口完成。C1a共享host-owned opaque authority与C5a纯network-failure/audit模块限定完成。C1b Parser接线以Parser 38 suites/925 tests、最终C1a+C1b定向2 suites/42 tests、typecheck/build及diff-check限定完成；C1c Gateway接线以4源码文件、Gateway 43 suites/649 tests、真实HTTP/TLS专项62/62、API build及diff-check限定完成；两侧在Resolver/body/cache前固定同一opaque handle、Snapshot、凭据、epoch及总deadline/abort，cache-off/单attempt保持，生产默认仍关闭。C1d的D1 host安全epoch/Registry提交事件合同以自身2 suites/16 tests、Parser 41 suites/971 tests（含C2a）、typecheck/build及diff-check限定完成；D2继续拆为D2a可信committed ACTIVE route目录/生命周期事件和D2b默认关闭的Gateway Registry/route/security epoch/Provider装配。D3 Parser host桥以5 Parser文件、专项20 tests、Parser 42 suites/991 tests、全量typecheck/build及diff-check限定完成；source/default-off、providerEvidence缺失永拒，WeakMap fixture仅为进程内不可伪造测试能力而非生产issuer，managed child/E3b与跨进程传播未接。D2a以SQL.js/旧快照2 suites/42 tests、Gateway 44 suites/664 tests、API build及diff-check限定完成；真实SQL.js dirty-read以同步export到独立query_only副本修复，PostgreSQL未实测、整库复制成本与原候选暂态snapshot行为保留。D2b进一步拆为b1 immutable host generation store、b2 Registry capture/opaque proof及b3 default-off Gateway/Parser host装配；b1以2个credentials文件、自身23 tests、Parser 44 suites/1077 tests、typecheck/build及diff-check限定完成，只保存内存有界材料并提供CAS激活/同步撤销到期，无env/file导入、生产issuer或Registry关联，JS string无物理擦除保证；b2 Registry capture/opaque proof以3个credentials文件、专用26 tests、相邻5 suites/99 tests、Parser 46 suites/1124 tests、typecheck/build、cleanup及diff-check限定完成；仅host-owned内存generation→Registry真实Snapshot→一次性source proof，无env/file自动捕获、Gateway生产装配、managed child或跨进程传播。首次TS7006阻断保留历史并已完整复验；b3再拆为a host-only generation/issuer capability端口、b Registry/evidence组合、c active-route registration协调器、d Nest/Gateway稳定provider装配；a以2个独立Gateway文件、最终Parser构建之上专项1 suite/5 tests、API typecheck/build及diff-check限定完成，创建端以Parser私有品牌拒绝克隆/代理/结构伪造及重复wrapper，一次性consume且close先同步失效再关闭issuer/abort，秘密不进入controller/error/JSON；b拆为b1本地安全组合与b2 PostgreSQL持久ledger环境验收；b1以2个独立Gateway文件、专项3 suites/44 tests、Gateway46 suites/683 tests及API typecheck/build限定完成，SQL.js共享事务CAS await交错导致rollback历史误公开已真实复现并改为boot fail-closed；b2限定DONE：本机原生PG16全新隔离集群完成14 warm+4 cold检查组，真实stop/restart与新进程冷开、同代多Provider/跨Provider Basic、坏配置/epoch/缺材料/伪proof拒绝、外部rollback隔离、双连接CAS/并发boot历史并集、revoke/close及schema零漂移均PASS；集群已停止/清理。首轮超时仅诊断记录。只证明本地隔离PG，外部目标部署/跨进程生产启用仍归F3D。c拆为c1 active-route原子捕获seam与c2可信registration协调器；c1以4个Gateway文件、新capture+catalog 2 suites/19 tests、route snapshot 1 suite/27 tests、Gateway全量47 suites/687 tests、API typecheck/build及diff-check限定完成，只接受exact current catalog对象并关联同次committed route引用，重核version/IDs/fingerprint，ID-only、克隆/旧snapshot拒绝且candidate/rollback/stop/delete/迟到reload围栏已验；c2现限定DONE，d拆为d1限定DONE/d2 READY，D4依d/D3。a缺失默认off且不从request/config/env/file自动导入，未改RuntimeModule/Registry或启用生产路径；外部Secret Manager、跨进程/E3b及目标环境证据归D4/F3D NEED_ENV。C2a精确method+path目标目录以2个独立Parser文件、专项30项及相邻5 suites/240 tests限定完成；C2b进一步拆为b1纯redirect chain state、b2原始Location证据与逐跳真实transport、b3 Transformer显式host配置/生产入口矩阵。b1以2个network文件、专项63 tests、相邻5 suites/303 tests、Parser 44 suites/1077 tests、typecheck/build及diff-check限定完成，只接受safe-read空正文GET/HEAD、规范化loop/max5和一次性decision；纯模块不触网、不启用生产入口。b2再拆为b2a rawHeaders唯一Location证据纯模块与b2b逐跳transport；b2b继续拆为b2b1 host-only同代readSignal及b2b2真实多跳transport。b2a以2个network文件、自身21 tests、相邻3 suites/115 tests、Parser 45 suites/1098 tests、typecheck/build、cleanup及diff-check限定完成；b2b1以evidence源码/spec两文件、专项2 suites/60 tests、Parser 46 suites/1135 tests、typecheck/build、cleanup及diff-check限定完成，同代generation与issuer失效合成同步AbortSignal，256有界缓存且终态detach；b2b2以4个Parser network文件、专项3 suites/87 tests、Parser 47 suites/1170 tests、typecheck/build、cleanup及diff-check限定完成；同一handle/signal/deadline与同generation贯穿逐跳DNS/TLS/epoch、raw Location、最多5跳及8MiB边界。b3以4个Parser文件、新增14 tests、Parser 48 suites/1184 tests、typecheck/build及diff-check限定完成；host-only safe-read只接受同一真实issuer与显式proof，工具/OpenAPI不能开关或提供目标模板。无redirect配置的受信network branch仍single-hop，历史Axios maxRedirects=5原样；Gateway未改、生产默认off且env/file不能激活。C4限定DONE：仅两份既有spec新增23项真实失败验收（Parser9、Gateway14）；专项27/76项、Parser48 suites/1193 tests、Gateway47 suites/701 tests、Parser typecheck/build、API build及diff-check均PASS。reset/503 Retry-After/DNS拒绝与不可用/TLS失败均单attempt，cache read/store零调用，失败lease不可重放且沿用原operation handle/deadline/signal；旧非网络cache/retry兼容保持。无生产源码或默认开关变更，不覆盖生产配置/DI、缓存恢复、自动retry、C3/C5b/C6或D2b3b2 PostgreSQL持久ledger；真实Provider撤销事件桥、原子revision、生产默认启用、缓存恢复/自动retry及F3D N01–N17环境矩阵继续等待，父TP-F3保持IN_PROGRESS。
