...
| TP-F3 | SEC-F03；C4/D1/E1 | IN_PROGRESS | F3-01政策已冻结；A纯compiler、B1受控DNS、B2≤8MiB Buffer单跳primitive与B3a共享private verified connection/Readable stream限定DONE。B3b Parser host桥以7文件、Parser 34 suites/838 tests、专项21、typecheck/build及diff-check限定DONE；真实Resolver Site/generation/revision（含None）、同Snapshot host policy/最终URL、bounded≤8MiB序列化及默认off/F1零发送已有证据。B3c Gateway route桥保持READY；生产Gateway/managed child、Provider撤销epoch及audit桥未接，C继续等待b/c，父B3/TP-F3及完整SSRF防线均未完成 |
