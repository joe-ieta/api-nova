...
| TP-F3 | SEC-F03；C4/D1/E1 | IN_PROGRESS | F3-01政策已冻结；A、B1、B2及B3a/b/c均限定DONE，B3聚合可限定关闭。B3c最终三态映射与HTTP/TLS专项46、相关6套111项、API build/diff-check通过；修复前Gateway43套627项不称最终一次全绿，Parser34套841项仍适用。默认off且无生产DI/Provider注册。C拆为C1/C2/C3/C4/C5a/C5b/C6，C1与C5a现READY并行，其余按依赖等待；整操作epoch/主动流中撤销、生产启用、缓存恢复/retry及F3D环境矩阵未完成，父TP-F3保持IN_PROGRESS |
