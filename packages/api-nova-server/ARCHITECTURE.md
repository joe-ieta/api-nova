...
`api-nova-server` is the runtime layer in a four-layer product structure:

- `api-nova-parser`: parse, validate, normalize, and extract OpenAPI/Swagger structure
- `api-nova-server`: transform parsed API operations into MCP tools and host MCP runtime transports
- `api-nova-api`: orchestrate runtime processes, persistence, security, and management APIs
- `api-nova-ui`: provide operator-facing import, preview, conversion, and management workflows
...
`api-nova-api` manages server lifecycle by spawning the runtime package through its built CLI entry. That means:
