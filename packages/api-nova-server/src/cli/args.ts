...
export function showHelp() {
  ...
  console.log(CliDesign.brand.info('  api-nova') + CliDesign.brand.muted(' [选项]'));
  ...
  console.log(CliDesign.example(
    'api-nova -t stdio -o https://petstore.swagger.io/v2/swagger.json',
    'STDIO 模式 - 适合 AI 客户端集成'
  ));
  console.log();

  console.log(CliDesign.example(
    'api-nova -t sse -p 3323 -o ./openapi.yaml -w',
    'SSE 模式 + 文件监控 - 适合开发环境'
  ));
  console.log();

  console.log(CliDesign.example(
    'api-nova -o ./api.json --auth-type bearer --bearer-token "your-token"',
    'Bearer Token 静态认证'
  ));
  console.log();

  console.log(CliDesign.example(
    'api-nova -o ./api.json --auth-type bearer --bearer-env API_TOKEN',
    'Bearer Token 环境变量认证'
  ));
  console.log();
  
  console.log(CliDesign.example(
    'api-nova -c ./config.json',
    '使用配置文件 - 支持完整配置'
  ));

  console.log(CliDesign.example(
    'api-nova -o ./api.json --custom-header "X-Client-ID=my-client" --custom-header-env "X-API-Key=API_KEY"',
    '自定义请求头 - 静态值与环境变量'
  ));
  console.log();

  console.log(CliDesign.example(
    'api-nova -o ./api.json --custom-headers-config ./headers.json --debug-headers',
    '自定义请求头 - 配置文件与调试模式'
  ));
  ...
  console.log('  ' + CliDesign.brand.muted('GitHub: ') + CliDesign.brand.info('https://github.com/joe-ieta/api-nova'));
  console.log('  ' + CliDesign.brand.muted('文档: ') + CliDesign.brand.info('https://github.com/joe-ieta/api-nova#readme'));
  
  console.log('\n' + CliDesign.separator('=', 60));
  console.log(CliDesign.brand.primary.bold('  感谢使用 ApiNova！'));
