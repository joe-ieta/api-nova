...
import type { AuthConfig } from 'api-nova-parser';
...
export class InteractiveCLI {
  ...
  private async showWelcome(): Promise<void> {
    ...
    const welcomeContent = `${chalk.hex(theme.colors.accent).bold.bgBlack(figlet.textSync('ApiNova (Mss)', { font: 'Small' }))}
...
${theme.icons.stats} 查看状态和统计信息`;;
  ...
  private async startServerWithConfig(config: SessionConfig): Promise<void> {
    if (config.transport === 'stdio') {
      await this.uiManager.showWarning(
        '交互式会话模式不支持 STDIO 启动。请使用 `api-nova --transport stdio --openapi <source>` 直接启动。'
      );
      return;
    }
  ...
  private async startDirectMode(): Promise<void> {
    try {
      ...
      console.log(chalk.cyan('🚀 ApiNova - 直接启动模式'));
      console.log();
      ...
      console.log(chalk.green('✓ ApiNova 启动成功！'));
