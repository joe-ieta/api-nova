...
describe('database options', () => {
  it('uses one complete entity registry and dialect-specific initial and forward migrations', () => {
    ...
    try {
      ...
      expect(options.migrations[6]).toContain('UpstreamProductionChallengeEvidenceSqlite');
      process.env.DB_TYPE = 'postgres';
      const postgres = buildDatabaseOptions();
      expect(postgres.migrations).toHaveLength(7);
      ...
      expect(postgres.migrations[6]).toContain('UpstreamProductionChallengeEvidencePostgres');
    } finally {
      process.env = saved;
