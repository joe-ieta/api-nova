...
// 通过 workspace 包导入 api-nova-server 的功能
import { 
  createMcpServer, 
...
/**
 * MCP 服务管理器 - 直接复用 api-nova-server 的核心能力
 * 
 * 这个服务不是通过子进程方式，而是直接在同一进程中使用 api-nova-server 的核心功能
 * 这样可以获得更好的性能和更简单的架构
 */
@Injectable()
export class MCPService implements OnModuleDestroy {
   ...
   * 直接使用 api-nova-server 的 createMcpServer 和 transformOpenApiToMcpTools
   */
  async createFromOpenAPI(
    ...
    try {
      this.logger.log('Creating MCP server from OpenAPI specification using api-nova-server core...');
      ...
      this.logger.log(`Generated ${tools.length} tools from OpenAPI spec using api-nova-server`);
  ...
  /**
   * 启动服务器 - 使用 api-nova-server 的启动函数
   */
    ...
    try {
      ...
      switch (transport) {
        case 'streamable':
          ...
          this.logger.log('Starting streamable server using api-nova-server...');
          this.httpServer = await startStreamableMcpServer(this.currentServer!, '/mcp', port);
          return `http://localhost:${port}/mcp`;
          
        case 'sse':
          // 使用 startSseMcpServer 直接启动现有服务器实例
          this.logger.log('Starting SSE server using api-nova-server...');
          setImmediate(() => {
  ...
  async reloadTools(openApiData: string | object): Promise<boolean> {
    try {
      this.logger.log('Reloading MCP tools using api-nova-server transformer...');
      
      // 重新生成工具 - 使用 api-nova-server 的核心函数
      const tools = await transformOpenApiToMcpTools(undefined, undefined, openApiData);
