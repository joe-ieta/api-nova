...
export const EVENT_QUERY_KEYS = ['after', 'until', 'eventTypes', 'severities', 'spanKinds', 'outcomes',
...
function parse(raw: Record<string, unknown>, previous: ObservabilityFilter = {}) {
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    if (['after', 'until', 'limit'].includes(key)) continue;
    if (ENUMS[key]) {
      const values = value.split(',');
      if (values.some(item => !ENUMS[key].includes(item)) || new Set(values).size !== values.length ||
        (key === 'serverType' && values.length !== 1)) throw new ObservabilityApiError('INVALID_QUERY', key);
...
export class CallObservabilityEventsService {
  ...
  private async read(raw: Record<string, unknown>, authorization: ObservabilityAuthorization) {
    ...
    return this.store.readSnapshot(async tx => {
      ...
      const start = after ? position(after) : '0';
      ...
      if (after && await range().andWhere('(event.expiresAt IS NULL OR event.expiresAt <= :now)', { now }).getExists()) {
