...
import {
  ...
  STAGED_SOURCE_RECOVERY_STATE_ID,
  type StagedSourceRecoveryReport,
...
export const STAGED_SOURCE_RECOVERY_WORKER_ID = STAGED_SOURCE_RECOVERY_STATE_ID;

export interface StagedSourceRecoveryWorkerConfiguration {
  enabled: boolean;
  intervalMs: number;
  scanLimit: number;
  deleteLimit: number;
...
export interface StagedSourceRecoveryWorkerReport {
  ...
  lastReport: StagedSourceRecoveryReport | null;
...
const PREFIX = 'API_NOVA_OBSERVABILITY_STAGED_SOURCE_';

export function stagedSourceRecoveryWorkerConfiguration(
  ...
  const retentionHours = positive('RETENTION_HOURS', 48, 48, 8760);
  return {
...
const STAGED_SOURCE_ERROR_CODES = [
  'INVALID_STAGED_SOURCE_RECOVERY_CONFIGURATION',
  'STAGED_SOURCE_RETENTION_BELOW_MINIMUM',
  'STAGED_SOURCE_RECOVERY_DISABLED',
  'STAGED_SOURCE_RECOVERY_WORKER_STOPPED',
  'STORAGE_BUSY',
  'UNSAFE_SOURCE_DIRECTORY',
  'UNSAFE_SOURCE_FILE',
  'SOURCE_FILE_CHANGED',
  'SOURCE_FILE_TRUNCATED',
  'SOURCE_SEALED_FILE_CHANGED',
  'STAGED_SOURCE_RECOVERY_FAILED',
] as const;
const SAFE_ERRORS = new Set<string>(STAGED_SOURCE_ERROR_CODES);

/** Default off. Only removes fully imported, sealed and aged staged source files. */
@Injectable()
export class CallObservabilityStagedSourceRecoveryWorker implements OnApplicationBootstrap, OnModuleDestroy {
  private timer?: ReturnType<typeof setTimeout>;
  private active?: Promise<StagedSourceRecoveryWorkerReport>;
  ...
  constructor(
    private readonly recovery: CallObservabilityStagedSourceRecoveryService,
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    let options: StagedSourceRecoveryWorkerConfiguration;
    try {
      options = stagedSourceRecoveryWorkerConfiguration(this.config);
    } catch (error) {
  ...
  runOnce(): Promise<StagedSourceRecoveryWorkerReport> {
    if (this.stopping) {
      return Promise.reject(new ObservabilityStorageError('STAGED_SOURCE_RECOVERY_WORKER_STOPPED'));
    }
  ...
  private async tick(): Promise<void> {
    try {
      ...
      if (this.stopping || !this.scheduled) return;
      try {
        const options = stagedSourceRecoveryWorkerConfiguration(this.config);
  ...
  private async run(): Promise<StagedSourceRecoveryWorkerReport> {
    let options: StagedSourceRecoveryWorkerConfiguration;
    try {
      options = stagedSourceRecoveryWorkerConfiguration(this.config);
      if (!options.enabled) {
        ...
        throw new ObservabilityStorageError('STAGED_SOURCE_RECOVERY_DISABLED');
      }
      await this.persist({
        state: 'running',
        workerConfigured: true,
      ...
      const report = await this.recovery.collect({
      ...
      if (error instanceof ObservabilityStorageError && error.code === 'STAGED_SOURCE_RECOVERY_DISABLED') {
        throw error;
      }
  ...
  private warn(): void {
    if (Date.now() - this.lastWarning >= 15000) {
      ...
      process.stderr.write('[OBSERVABILITY_STAGED_SOURCE_DEGRADED] Staged source recovery deferred; inspect persisted worker evidence.\n');
    }
  }
  ...
  private failure(
    error: unknown,
    options: StagedSourceRecoveryWorkerConfiguration | null,
  ): Promise<StagedSourceRecoveryWorkerReport> {
    const code = error instanceof ObservabilityStorageError && SAFE_ERRORS.has(error.code)
      ? error.code : 'STAGED_SOURCE_RECOVERY_FAILED';
    return this.persist({
      state: 'degraded',
      workerConfigured: options?.enabled ?? false,
      intervalMs: options?.intervalMs ?? null,
      retentionHours: options?.retentionHours ?? null,
  ...
  private persist(patch: Partial<StagedSourceRecoveryWorkerReport>): Promise<StagedSourceRecoveryWorkerReport> {
    return this.store.transaction(async tx => {
      const repository = tx.manager.getRepository(RuntimePipelineStateEntity);
      const previous = await repository.findOneBy({ id: STAGED_SOURCE_RECOVERY_WORKER_ID });
      ...
      const value: StagedSourceRecoveryWorkerReport = {
        state: 'disabled',
        workerConfigured: false,
        evidenceScope: 'staged_source_recovery',
        intervalMs: null,
      ...
      await repository.save(repository.create({
        id: STAGED_SOURCE_RECOVERY_WORKER_ID,
        value,
        updatedAt: tx.now,
      }));
      this.observed = true;
...
export { STAGED_SOURCE_MIN_RETENTION_MS };
