...
export class CallObservabilityStatisticsService {
  ...
  private readPersistedBucketPayload(row: RuntimeMetricBucketEntity): StoredBucketPayload | null {
    ...
    if (raw.metrics && raw.coverage) {
      return this.hasPersistedCacheMetrics(raw.metrics) ? { metrics: raw.metrics, coverage: raw.coverage } : null;
    }
    if (raw.selectedInvocations !== undefined && raw.coverage) {
      return this.hasPersistedCacheMetrics(raw) ? { metrics: raw, coverage: raw.coverage } : null;
    }
    return null;
  }

  private hasPersistedCacheMetrics(metrics: any): boolean {
    // Legacy buckets did not persist cache evidence. Fall back to the existing retained-detail
    // path rather than inventing zero hits/misses or mixing metrics from different populations.
    if (!metrics || typeof metrics !== 'object' || Array.isArray(metrics)) return false;
    const counters = ['cacheEligibleRecords', 'cacheObservedRecords', 'cacheUnknownRecords', 'cacheHits', 'cacheMisses'];
    if (counters.some(field => !Number.isSafeInteger(metrics[field]) || metrics[field] < 0)) return false;
    const { cacheEligibleRecords: eligible, cacheObservedRecords: observed, cacheUnknownRecords: unknown,
      cacheHits: hits, cacheMisses: misses, cacheHitRate: rate, cacheCoveragePartial: partial } = metrics;
    return hits + misses === observed && observed + unknown === eligible &&
      partial === (unknown > 0) && (observed === 0 ? rate === null : rate === hits / observed);
  }
