...
describe('MCP stale activation guard with SQL.js transaction rollback', () => {
  ...
  beforeEach(async () => {
    ...
    await db.initialize();
    await db.getRepository(RuntimeAssetEntity).save({ id: id(1), name: 'fixture', type: 'mcp_server' as any, metadata: { activeRevision: 'prior', managedServerId: id(2) } });
    await db.getRepository(MCPServerEntity).save({ id: id(2), name: 'server', openApiData: { original: true }, config: { runtimeAssetId: id(1), endpoint: '/mcp' } });
  ...
  it.each(['revision', 'missing', 'membership', 'inactive', 'active-changed', 'invalidated', 'missing-invalidation-time', 'duplicate', 'invalid-revision', 'missing-list'])('rejects %s and rolls back preceding server writes', async reason => {
    ...
    if (reason === 'active-changed') await db.getRepository(RuntimeAssetEntity).update(id(1), { metadata: { activeRevision: 'newer', managedServerId: id(2) } });
    if (reason === 'invalidated') await db.getRepository(RuntimeAssetEntity).update(id(1), { metadata: { activeRevision: 'prior', managedServerId: id(2), verificationRequired: true, verificationRequiredAt: '2026-09-15T00:00:01Z' } });
    if (reason === 'missing-invalidation-time') await db.getRepository(RuntimeAssetEntity).update(id(1), { metadata: { activeRevision: 'prior', managedServerId: id(2), verificationRequired: true } });
  ...
  it.each(['port', 'transport', 'path', 'missing-config', 'missing-field'])('rejects endpoint %s drift and rolls back replacement configuration', async reason => {
    if (reason === 'missing-config' || reason === 'missing-field') await db.getRepository(RuntimeVerificationRunEntity).update(id(6), {
      metadata: reason === 'missing-config' ? {} : { mcpEndpointConfig: { port: 9022, transport: 'streamable' } },
    });
    await expect(db.transaction(async manager => {
      await manager.getRepository(MCPServerEntity).update(id(2), {
        port: reason === 'port' ? 9023 : 9022,
        transport: (reason === 'transport' ? 'sse' : 'streamable') as any,
        config: { runtimeAssetId: id(1), endpoint: reason === 'path' ? '/changed' : '/mcp' } as any,
      });
      await service.activateMcpCandidate(id(1), id(6), manager);
    })).rejects.toThrow('MCP_CANDIDATE_STALE');
    const server = await db.getRepository(MCPServerEntity).findOneByOrFail({ id: id(2) });
    expect(server.port).toBe(9022); expect(server.transport).toBe('streamable'); expect(server.config.endpoint).toBe('/mcp');
    expect((await db.getRepository(RuntimeAssetEntity).findOneByOrFail({ id: id(1) })).metadata.activeRevision).toBe('prior');
  });
