'use strict';
// SEC-D1-02D4D2: Gateway history bootstrap cold-process HTTP in a disposable PostgreSQL cluster.
...
async function smoke(port) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ['scripts/test-postgres-header-history-bootstrap.cjs'], {
    ...
    child.once('close', code => {
      ...
      try {
        const line = output.split(/\r?\n/).find(line => line.startsWith('{"marker":"POSTGRES_HEADER_HISTORY_BOOTSTRAP_OK"'));
        assert.ok(line, 'Missing complete database acceptance report');
        const report = JSON.parse(line);
        assert.equal(report.dialect, 'postgres');
        for (const key of ['coldProcesses', 'realHttp', 'clearedRetained', 'policyRejected', 'zeroDrift', 'noActivation']) assert.equal(report[key], true);
        resolve(report);
...
async function main() {
  ...
  directory = await fs.mkdtemp(path.join(scratchRoot, 'pg-header-history-bootstrap-'));
  ...
  try {
    ...
    assert.ok(path.basename(target).startsWith('pg-header-history-bootstrap-'));
    assert.equal(started, false, 'Refusing to delete a running cluster');
    await fs.rm(target, { recursive: true, force: true });
  }
  console.log(JSON.stringify({ ...report,
    marker: 'ISOLATED_POSTGRES_HEADER_HISTORY_BOOTSTRAP_OK', clusterStopped: true, clusterRemoved: true,
    scope: 'header-history-bootstrap-two-processes-http-clear-rotation-full-schema' }));
}
