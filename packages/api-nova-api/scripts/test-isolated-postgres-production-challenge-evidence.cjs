'use strict';
// SEC-D1-02D2D: Production evidence schema in a disposable PostgreSQL cluster.
...
async function smoke(port) {
  const env = { ...environment, API_NOVA_ISOLATED_PRODUCTION_EVIDENCE: 'true', NODE_ENV: 'test', DB_TYPE: 'postgres', DB_HOST: '127.0.0.1',
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ['scripts/test-postgres-production-challenge-evidence.cjs'], {
    ...
    child.once('close', code => {
      ...
      try {
        const line = output.split(/\r?\n/).find(line => line.startsWith('{"marker":"POSTGRES_PRODUCTION_EVIDENCE_OK"'));
        assert.ok(line, 'Missing complete database acceptance report');
        const report = JSON.parse(line);
        assert.equal(report.dialect, 'postgres');
        for (const key of ['checks', 'reopen', 'expiry', 'revocation', 'prototypeIsolation', 'noProof', 'reversible']) assert.equal(report[key], true);
        resolve(report);
...
async function main() {
  ...
  directory = await fs.mkdtemp(path.join(scratchRoot, 'pg-production-evidence-'));
  ...
  try {
    ...
    assert.ok(path.basename(target).startsWith('pg-production-evidence-'));
    assert.equal(started, false, 'Refusing to delete a running cluster');
    await fs.rm(target, { recursive: true, force: true });
  }
  console.log(JSON.stringify({ ...report,
    marker: 'ISOLATED_POSTGRES_PRODUCTION_EVIDENCE_OK', clusterStopped: true, clusterRemoved: true,
    scope: 'production-challenge-evidence-migrations-reopen-checks-no-proof' }));
}
