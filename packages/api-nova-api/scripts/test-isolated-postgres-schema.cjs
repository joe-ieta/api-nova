'use strict';
// SEC-A4-02: complete current-schema acceptance in a disposable PostgreSQL cluster.
...
async function smoke(port) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ['scripts/database-tool.cjs', 'smoke', 'postgres'], {
    ...
    child.once('close', code => {
      ...
      try {
        const line = output.split(/\r?\n/).find(line => line.startsWith('{"marker":"DATABASE_SMOKE_OK"'));
        assert.ok(line, 'Missing complete database acceptance report');
        const report = JSON.parse(line);
        assert.equal(report.dialect, 'postgres');
        assert.equal(report.schemaDrift, 0); assert.equal(report.restartSchemaDrift, 0);
        assert.equal(report.restartMigrations, 0);
        for (const key of ['empty', 'restart', 'persistence', 'apiStartup']) assert.equal(report[key], true);
        resolve(report);
...
async function main() {
  ...
  directory = await fs.mkdtemp(path.join(scratchRoot, 'pg-schema-acceptance-'));
  ...
  try {
    ...
    assert.ok(path.basename(target).startsWith('pg-schema-acceptance-'));
    assert.equal(started, false, 'Refusing to delete a running cluster');
    await fs.rm(target, { recursive: true, force: true });
  }
  console.log(JSON.stringify({ ...report,
    marker: 'ISOLATED_POSTGRES_SCHEMA_ACCEPTANCE_OK', clusterStopped: true, clusterRemoved: true,
    scope: 'local-current-schema-migrations-reconnect-persistence-api-startup' }));
}
