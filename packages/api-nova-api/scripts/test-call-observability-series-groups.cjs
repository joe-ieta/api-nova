...
test('capabilities advertise the implemented series and grouping bounds without enabling future push', async t => {
  ...
  assert.equal(feature(value, 'webhook').state, 'restricted');
  assert.equal(feature(value, 'webhook').scopeMode, 'none');
  assert.equal(feature(value, 'socketPush').state, 'not_implemented');
});
