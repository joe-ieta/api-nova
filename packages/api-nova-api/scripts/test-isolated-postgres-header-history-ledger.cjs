'use strict';
// SEC-D1-02D4D2: Header-name history ledger CAS in a disposable PostgreSQL cluster.
...
async function smoke(port) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, ['scripts/test-postgres-header-history-ledger.cjs'], {
    ...
    child.once('close', code => {
      ...
      try {
        const line = output.split(/\r?\n/).find(line => line.startsWith('{"marker":"POSTGRES_HEADER_HISTORY_LEDGER_OK"'));
        assert.ok(line, 'Missing complete database acceptance report');
        const report = JSON.parse(line);
        assert.equal(report.dialect, 'postgres');
        for (const key of ['concurrentCas', 'reopen', 'checks', 'oldMigration', 'zeroDrift', 'noActivation']) assert.equal(report[key], true);
        resolve(report);
...
async function main() {
  ...
  directory = await fs.mkdtemp(path.join(scratchRoot, 'pg-header-history-ledger-'));
  ...
  try {
    ...
    assert.ok(path.basename(target).startsWith('pg-header-history-ledger-'));
    assert.equal(started, false, 'Refusing to delete a running cluster');
    await fs.rm(target, { recursive: true, force: true });
  }
  console.log(JSON.stringify({ ...report,
    marker: 'ISOLATED_POSTGRES_HEADER_HISTORY_LEDGER_OK', clusterStopped: true, clusterRemoved: true,
    scope: 'header-history-ledger-old-migration-cas-reopen-checks-full-schema' }));
}
