...
test('private_api_key persisted lifecycle survives disk reopen and enforces API keys', { timeout: 60000 }, async t => {
  ...
  const port = await persistedLaunch(t, 'private_api_key', inherited, { 'x-api-key': key });
...
test('private_jwt persisted lifecycle survives disk reopen and enforces JWTs', { timeout: 60000 }, async t => {
  ...
  const port = await persistedLaunch(t, 'private_jwt', inherited, { authorization: 'Bearer ' + token });
  assert.equal((await request(port)).status, 401);
...
test('anonymous persisted lifecycle survives disk reopen with explicit anonymous', { timeout: 60000 }, async t => {
  const inherited = {
    ...process.env,
  ...
  const port = await persistedLaunch(t, 'anonymous', inherited, {});
...
async function persistedLaunch(t, mode, inherited, credentials) {
  ...
  await open();
  const saved=await publication.publish(db,port,specPort,mode,status=>{upstreamStatus=status;});
