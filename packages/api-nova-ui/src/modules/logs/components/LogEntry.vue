<template>
  <div class="log-entry" :class="[`log-level-${log.level}`, { expanded }]">
    <div class="log-main" @click="toggleExpanded">
      ...
      <div v-if="log.tags && log.tags.length > 0" class="log-tags">
        <el-tag v-for="tag in log.tags" :key="tag" size="small" class="tag-item">
          {{ tag }}
        </el-tag>
      </div>
...
const copyLog = async () => {
  try {
    const logText = [
      ...
      ...(props.log.context
        ? [`上下文: ${JSON.stringify(props.log.context, null, 2)}`]
        : []),
