              ...
              >
                ...
                <div class="config-details">
                  ...
                  <div class="detail-item">
                    ...
                    <span
                      class="value"
                      :class="config.valid ? 'valid' : 'invalid'"
                    >
                      {{ config.valid ? "有效" : "无效" }}
                    </span>
                  </div>
          ...
          <div v-if="importForm.file" class="file-info">
            ...
            <div class="file-details">
              ...
              <div class="detail-item">
                ...
                <span class="value">{{
                  getFileType(importForm.file.name)
                }}</span>
              </div>
            </div>
          </div>
      ...
      <el-dialog v-model="historyDetailsVisible" title="操作详情" width="600px">
        <div v-if="selectedHistoryItem" class="history-details">
          <div class="detail-section">
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                ...
                <span
                  class="value"
                  :class="selectedHistoryItem.success ? 'success' : 'error'"
                >
          ...
          <div class="detail-section" v-if="selectedHistoryItem.details">
            <h4>详细信息</h4>
            <pre>{{
              JSON.stringify(selectedHistoryItem.details, null, 2)
            }}</pre>
          </div>
        </div>
      </el-dialog>
...
const handleMigrationCompleted = (result: any) => {
  migrationWizardVisible.value = false;
  ElMessage.success(
    `配置迁移完成: ${result.fromVersion} → ${result.toVersion}`,
  );
