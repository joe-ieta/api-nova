            ...
            <div class="card-content">
              ...
              <div class="credentials-summary">
                <div v-if="config.type === 'bearer'" class="credential-item">
                  ...
                  <span class="value">{{
                    maskCredential(config.credentials.token)
                  }}</span>
                </div>
                <div
                  v-else-if="config.type === 'apikey'"
                  class="credential-item"
                >
                  <span class="label">API Key:</span>
                  <span class="value">{{
                    maskCredential(config.credentials.apiKey)
                  }}</span>
                </div>
                <div v-else-if="config.type === 'basic'">
                  ...
                  <div class="credential-item">
                    ...
                    <span class="value">{{
                      maskCredential(config.credentials.password)
                    }}</span>
                  </div>
                </div>
                <div v-else-if="config.type === 'oauth2'">
                  ...
                  <div class="credential-item">
                    ...
                    <span class="value">{{
                      maskCredential(config.credentials.clientSecret)
                    }}</span>
                  </div>
                </div>
              </div>
              ...
              <!-- 环境变量 -->
              <div
                v-if="config.envVars && config.envVars.length > 0"
                class="env-vars"
              >
              ...
              <div v-if="testResults.get(config.id)" class="test-result">
                <div class="result-header">
                  ...
                  <el-tag
                    :type="
                      testResults.get(config.id)?.success ? 'success' : 'danger'
                    "
                    size="small"
                  >
                  ...
                  <span class="result-time">
                    {{
                      formatDateTime(
                        testResults.get(config.id)?.timestamp || new Date(),
                      )
                    }}
                  </span>
                </div>
    ...
    >
      ...
      >
        <el-form-item label="配置名称" prop="name">
          <el-input v-model="authForm.name" placeholder="输入认证配置名称" />
        </el-form-item>
        ...
        <el-form-item label="环境变量">
          <div class="env-vars-config">
            ...
            <div class="env-vars-list" v-if="authForm.envVars.length > 0">
              ...
              >
                {{ envVar }}
                <el-icon v-if="getEnvVarStatus(envVar) === 'success'"
                  ><Check
                /></el-icon>
                <el-icon v-else-if="getEnvVarStatus(envVar) === 'danger'"
                  ><Close
                /></el-icon>
              </el-tag>
            </div>
            ...
            <div v-if="envVarValidationResult" class="env-validation-result">
              <div
                v-if="envVarValidationResult.available.length > 0"
                class="available-vars"
              >
                <el-text type="success">
                  <el-icon><Check /></el-icon>
                  可用环境变量:
                  {{ envVarValidationResult.available.join(", ") }}
                </el-text>
              </div>
              <div
                v-if="envVarValidationResult.missing.length > 0"
                class="missing-vars"
              >
                <el-text type="danger">
          ...
          <div class="auto-renewal-config">
            ...
            <div v-if="authForm.autoRenewal" class="renewal-settings">
              <el-form-item label="提醒间隔" style="margin-bottom: 8px">
                <el-select
                  v-model="authForm.renewalInterval"
                  style="width: 100%"
                >
...
const filteredConfigs = computed(() => {
  ...
  return authStore.configList.filter(
...
const editConfig = (config: { id: string } & AuthConfig) => {
  ...
  const decryptedCredentials =
    authStore.getDecryptedCredentials(config.id) || {};
  authForm.value = {
