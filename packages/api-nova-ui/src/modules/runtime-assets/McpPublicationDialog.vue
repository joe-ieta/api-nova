<template>
  <el-dialog :model-value="state.visible" :title="t('monitoring.mcpPublication.title')" width="560px" :close-on-click-modal="false" @close="form.close()">
    ...
    <template #footer>
      ...
      <el-button type="primary" :loading="state.saving" :disabled="!state.preview || state.previewLoading || state.loading" @click="form.save()">{{ t('monitoring.mcpPublication.deploy') }}</el-button>
    </template>
  </el-dialog>
</template>
...
watch([() => state.transport, () => state.port, () => state.endpointPath], () => { void form.refresh(); }, { flush: 'sync' });
