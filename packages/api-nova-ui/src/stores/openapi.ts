...
export const useOpenAPIStore = defineStore("openapi", () => {
  ...
  const fetchSpecs = async (): Promise<void> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "获取规范列表失败";
  ...
  const getSpecContent = async (id: string): Promise<string> => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "获取规范内容失败";
      setError(errorMessage);
  ...
  // 验证OpenAPI内容
  const validateOpenAPIContent = async (
    content: string,
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "验证OpenAPI内容失败";
      setError(errorMessage);
  ...
  // 解析OpenAPI内容
  const parseOpenAPIContent = async (
    content: string,
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "解析OpenAPI内容失败";
      setError(errorMessage);
  ...
  // 从URL解析OpenAPI
  const parseOpenAPIFromUrl = async (
    url: string,
  ): Promise<ParsedOpenAPISpec> => {
    setLoading(true);
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "从URL解析OpenAPI失败";
      setError(errorMessage);
  ...
  // 新增：上传并验证 OpenAPI 文件
  const uploadAndValidateSpec = async (
    file: File,
  ): Promise<ValidationResult> => {
    setLoading(true);
    try {
  ...
  const validateSpecFromUrl = async (
    url: string,
  ): Promise<ValidationResult> => {
    setLoading(true);
    try {
