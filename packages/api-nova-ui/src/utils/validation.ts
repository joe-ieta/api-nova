...
export const validateOpenAPIUrl = async (
  ...
  try {
    ...
    const validator = new Validator(parserConfig);
    const result: ParserValidationResult = await validator.validate(
      parseResult.spec,
    );
...
export const validateConfigFile = (
  config: any,
): { isValid: boolean; errors: UIValidationError[] } => {
  const errors: UIValidationError[] = [];
...
export const detectConfigConflicts = (
  ...
  importedServers.forEach((importedServer) => {
    ...
    if (existingServer) {
      ...
      // 检查配置冲突
      if (
        JSON.stringify(existingServer.config) !==
        JSON.stringify(importedServer.config)
      ) {
        conflicts.push({
          type: "server",
          id: importedServer.id,
