...
export class GatewaySecurityService {
  ...
  async authorize(
    ...
    const visibility = String((resolvedRoute.routeBinding as { routeVisibility?: string }).routeVisibility || 'internal');
    const mode =
      configuredMode === 'anonymous' && visibility === 'internal' ? 'jwt' : configuredMode;
    if (mode === 'anonymous') {
