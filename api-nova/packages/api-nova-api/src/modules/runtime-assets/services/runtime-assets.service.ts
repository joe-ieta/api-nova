...
export class RuntimeAssetsService {
  ...
  async assembleMcpRuntimeAssetPayload(runtimeAssetId: string) {
    ...
    for (const item of includedMemberships) {
      ...
      const operation = {
        ...rawOperation,
        operationId: this.sanitizeOperationId(
  ...
  private sanitizeOperationId(operationId?: string) {
    const cleaned = String(operationId || '')
      .trim()
  ...
  private buildGatewayRouteAccessUrls(route: Record<string, any>, runtimeEndpoint?: string) {
    ...
    if (values.length === 0) {
      const publicBase = this.resolveGatewayPublicBaseUrl();
      const prefix = publicBase
        ? `${publicBase.replace(/\/+$/, '')}/api/v1/gateway`
        : '/api/v1/gateway';
      values.push(`${prefix}${routePath}`);
    }

    return values;
  }

  private resolveGatewayPublicBaseUrl() {
    return String(this.appConfigService.apiBaseUrl || '').trim();
  }
