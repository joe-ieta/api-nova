...
export class AssetCatalogService {
  ...
  async executeEndpointDefinitionTest(
    ...
    const rawTestUrl = `${this.buildBaseUrlFromSourceServiceAsset(sourceServiceAsset)}${endpoint.path}`;
    const testUrl = this.substitutePathParameters(rawTestUrl, endpoint);
  ...
  async updateManualEndpointAssetRecord(id: string, input: UpdateManualEndpointAssetDto) {
    ...
    endpoint.description = input.description;
    endpoint.rawOperation = this.buildManualOperationTemplate(input.parameters, input.requestBody);
    endpoint.metadata = {
      ...(endpoint.metadata || {}),
      source: 'manual-registration',
      businessDomain: input.businessDomain,
    ...
    await this.syncEndpointRouteBindings(endpoint);
    await this.endpointDefinitionRepository.save(endpoint);
  ...
  private buildManualOperationTemplate(
    ...
    requestBody?: ManualEndpointRequestBodyDto,
    ...
    if (normalizedParameters.length > 0) {
      template.parameters = normalizedParameters
        ...
        .map(param => ({
          ...
          schema: {
            ...(param.type ? { type: param.type } : { type: param.in === 'path' ? 'string' : 'string' }),
    ...
    if (!template.parameters && !template.requestBody) {
      return undefined;
    }
    return template;
  }

  private substitutePathParameters(url: string, endpoint: EndpointDefinitionEntity) {
    if (!/\{[^}]+\}/.test(url)) {
      return url;
    }
    return url.replace(/\{([^}]+)\}/g, (match, paramName: string) => {
      if (paramName === 'proxy' || paramName === 'path' || paramName === 'splat') {
        return match;
      }
      return encodeURIComponent(String(this.resolvePathSample(String(paramName), endpoint)));
    });
  }

  private resolvePathSample(paramName: string, endpoint: EndpointDefinitionEntity) {
    const metadata = (endpoint.metadata || {}) as Record<string, unknown>;
    const testParameters =
      metadata.testParameters && typeof metadata.testParameters === 'object'
        ? (metadata.testParameters as Record<string, unknown>)
        : {};
    if (testParameters[paramName] !== undefined) {
      return testParameters[paramName];
    }

    const rawOperation = (endpoint.rawOperation || {}) as Record<string, unknown>;
    const parameters = Array.isArray(rawOperation.parameters) ? rawOperation.parameters : [];
    const param = parameters.find(
      (candidate: any) =>
        candidate && candidate.name === paramName && (candidate.in === 'path' || candidate.in === 'query'),
    ) as {
      example?: unknown;
    ...
    if (paramName === 'id' || paramName === 'key' || paramName === 'userId') {
      return '1';
    }
    return 'sample';
  }

  private async hasActivePublicationBinding(endpoint: EndpointDefinitionEntity) {
    const bindings = await this.publishBindingRepository.find({
      where: {
        endpointDefinitionId: endpoint.id,
  ...
  private async ensureManualEndpointMutatable(endpoint: EndpointDefinitionEntity) {
    if (await this.hasActivePublicationBinding(endpoint)) {
      throw new ConflictException(
        `Manual endpoint '${endpoint.id}' has active publication bindings; offline its published memberships before changing method/path/baseUrl`,
      );
    }
  }
  ...
  private async ensureManualEndpointRemovable(endpoint: EndpointDefinitionEntity) {
    if (await this.hasActivePublicationBinding(endpoint)) {
      throw new ConflictException(
        `Manual endpoint '${endpoint.id}' has active publication bindings; offline its published memberships before deleting the endpoint`,
      );
    }
  }

  private async syncEndpointRouteBindings(endpoint: EndpointDefinitionEntity) {
    const routeBindings = await this.routeBindingRepository.find({
      where: { endpointDefinitionId: endpoint.id },
    });
    for (const binding of routeBindings) {
      if (binding.status === GatewayRouteBindingStatus.ACTIVE) {
        continue;
      }
      binding.routePath = this.normalizeBasePath(endpoint.path);
      binding.upstreamPath = this.normalizeBasePath(endpoint.path);
      binding.routeMethod = endpoint.method.toUpperCase();
      binding.upstreamMethod = endpoint.method.toUpperCase();
  ...
  private async cascadeDeleteEndpointRecord(endpoint: EndpointDefinitionEntity) {
    ...
    const membershipIds = memberships.map(member => member.id).filter(Boolean);
    ...
    await this.endpointDefinitionRepository.manager.transaction(async manager => {
      ...
      const routeBindingRepository = manager.getRepository(GatewayRouteBindingEntity);
      const profileRepository = manager.getRepository(PublicationProfileEntity);
      const profileHistoryRepository = manager.getRepository(PublicationProfileHistoryEntity);
      const publicationAuditRepository = manager.getRepository(PublicationAuditEventEntity);
      const runtimeBindingRepository = manager.getRepository(RuntimeAssetEndpointBindingEntity);
      const endpointDefinitionRepository = manager.getRepository(EndpointDefinitionEntity);

      await publishBindingRepository.delete({ endpointDefinitionId: endpoint.id });
      await routeBindingRepository.delete({ endpointDefinitionId: endpoint.id });
      await profileHistoryRepository.delete({ endpointDefinitionId: endpoint.id });
      await profileRepository.delete({ endpointDefinitionId: endpoint.id });
      await publicationAuditRepository.delete({ endpointDefinitionId: endpoint.id });

      if (membershipIds.length > 0) {
        await publicationAuditRepository.delete({
          runtimeAssetEndpointBindingId: In(membershipIds),
        });
        await routeBindingRepository.delete({
          runtimeAssetEndpointBindingId: In(membershipIds),
        });
        ...
        await runtimeBindingRepository.delete({ endpointDefinitionId: endpoint.id });
      }

      await endpointDefinitionRepository.delete({ id: endpoint.id });
    });
  }
